...
const STORAGE_SEARCH_KEY = 'currSearch';
...
function add(bookToSave) {

    (gBooks) ? gBooks.unshift(bookToSave) : gBooks = [bookToSave];
    storageService.store(STORAGE_KEY, gBooks);
}
...
function getBookApiInfo(name) {

    return fetch(`https://www.googleapis.com/books/v1/volumes?q=${name}&key=AIzaSyDhs5CgUBpYIb22Hm2DV_ESjLsOKSDnQpU`)
        .then(res => res.json())
        .then(
            (result) => {
                console.log(result);
                storageService.store(STORAGE_SEARCH_KEY, result);
